      ...
      <main>
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
          <div class="px-4 py-8 sm:px-0">
                ...
                <div class="flex gap-4 items-end">
                  <div class="flex-1 max-w-sm">
                    ...
                    <input
                      ...
                      type="number"
                      placeholder="Enter recipe ID (e.g., 100)"
            ...
            <!-- Step 2: Portion Measurements Confirmation (if recipe has product) -->
            <div v-if="pendingResult && pendingResult.has_product && !portionConfirmed" class="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-6">
              <div class="flex">
                <div class="flex-shrink-0">
                ...
                <div class="ml-3 flex-1">
                  <h3 class="text-sm font-medium text-yellow-800">Important Question</h3>
                  <div class="mt-2">
                    <p class="text-sm text-yellow-700">
                      This recipe has an associated product:
                      <a
                    ...
                    <p class="text-sm text-yellow-600 mt-1">
                      If you modified the number of servings or portion sizes in Grocy, click "Yes" to continue.
                      Otherwise, click the link above to update the product first.
                    </p>
                  </div>
                  <div class="mt-4 flex gap-3">
                    ...
                    <button
                      @click="confirmPortionMeasurements(false)"
...
const calculateNutrients = async () => {
  ...
  try {
    ...
    // If recipe has associated product, ask for confirmation first
    if (response.data.has_product) {
      pendingResult.value = response.data
    } else {
      // No product - show results directly
...
const confirmPortionMeasurements = (confirmed: boolean) => {
  if (!confirmed) {
    // User didn't change measurements - reset and don't show results
    error.value = 'Please update recipe portion measurements in Grocy first, then try again.'
    pendingResult.value = null
    portionConfirmed.value = false
  } else {
    // User confirmed - show the results
