    ...
    <div class="border-t border-gray-200 px-4 py-5 sm:px-6">
      ...
      >
        ...
        >
          ...
          <div
            v-if="expandedSync === h.id && h.role_name === 'admin' && h.grocy_url"
            class="mt-3 bg-gray-50 rounded-lg p-4"
          >
            <div
              v-if="sync.error.value && sync.householdId.value === h.id"
              class="mb-2 text-xs text-red-500"
            >
              {{ sync.error.value }}
            </div>
            <div
              v-if="sync.success.value && sync.householdId.value === h.id"
              class="mb-2 text-xs text-green-600"
            >
              {{ sync.success.value }}
            </div>
            <div class="flex items-center gap-3">
              ...
              <button
                @click="sync.syncStockExpiry(h.id)"
                :disabled="sync.loading.value"
                class="inline-flex items-center py-1.5 px-3 border border-transparent shadow-xs text-xs font-medium rounded-md text-white bg-amber-600 hover:bg-amber-700 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <svg
                  v-if="sync.loading.value && sync.type.value === 'stock' && sync.householdId.value === h.id"
                  class="animate-spin -ml-0.5 mr-1.5 h-3 w-3 text-white"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                ...
                {{ sync.loading.value && sync.type.value === 'stock' && sync.householdId.value === h.id ? 'Syncing...' : 'Sync Expiring Stock' }}
              </button>
          ...
          <!-- Members panel -->
          <div
            v-if="expandedMembers === h.id && householdStore.householdDetails[h.id]"
            class="mt-3"
          >
            <table class="min-w-full divide-y divide-gray-200 border rounded-md overflow-hidden">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr
                  v-for="m in householdStore.householdDetails[h.id].members"
                  :key="m.user_id"
                >
                  <td class="px-4 py-3">
                  ...
                  <td class="px-4 py-3">
                    <template v-if="m.user_id === authStore.user?.id">
                      <div class="flex items-center space-x-2">
                        <span
                          v-if="m.has_grocy_key"
                          class="inline-flex items-center text-xs text-green-700"
                        >
                          <svg
                            class="h-4 w-4 mr-1"
                            fill="currentColor"
                      ...
                      <p
                        v-if="grocyKey.target.value?.householdId === h.id && grocyKey.target.value?.userId === m.user_id && grocyKey.error.value"
            ...
            >
              ...
              <ul
                v-if="memberSearch.results.value.length > 0"
                class="mt-1 border rounded-md divide-y divide-gray-200 max-h-40 overflow-y-auto"
              >
                <li
              ...
              <p
                v-if="memberSearch.error.value"
                class="mt-1 text-xs text-red-500"
              >
                {{ memberSearch.error.value }}
              </p>
            </div>
          </div>
        </div>
